...
def fen_line_extract_train(fen_data):
    parser = FENParser()
    encoder = FENEncoder()
    ...
    board = ['b' if side_to_move == 1.0 else 'w', [], victory]
...
def fen_parser_to_data(fen_file, train):
    results = []
    with open(fen_file, "r") as f:
        for line_number, line in enumerate(f, start=1):
            ...
            try:
                if train:
                    board = fen_line_extract_train(fen)
                else:
